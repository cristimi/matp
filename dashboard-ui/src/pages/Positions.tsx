      ...
      {!loading && positions.length > 0 && (
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-gray-500 uppercase border-b border-gray-200 dark:border-gray-800">
                  {['Status', 'Symbol', 'Side', 'Size', 'Entry', 'Mark', 'Close', 'Unrealized P&L', 'Platform', ''].map((h) => (
                ...
                {positions.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="table-row-hover">
                      ...
                      <td className="px-4 py-3 font-mono text-gray-600 dark:text-gray-300">{formatPrice(p.symbol, p.closePx)}</td>
          ...
          <div className="md:hidden space-y-3">
            {positions.map((p) => (
              <div key={p.id} className="stat-card space-y-3 shadow-sm transition-colors">
                <div className="flex items-center justify-between">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <p className="font-mono text-gray-700 dark:text-gray-300">{formatSize(p.symbol, p.size)}</p>
                  </div>
                  <div>
