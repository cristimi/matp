...
async def fetch_fear_greed() -> dict | None:
    ...
    Process-wide cached (see app.data.cache) — the index moves at most
    once/day, but node_ingest's concurrent per-strategy fan-out was hitting
    this with a fresh connection from every strategy every cycle.
