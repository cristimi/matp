...
| **How much did entry slippage cost?** | Market orders store no intended reference price. | Persist the decision-time mark price on the order alongside `actual_fill_price`. **Correction (2026-07-28):** this row originally suggested reusing `orders.indicator_price` as "exists but is unused". That is true of the *data* (0 of 508 rows populated) but false of the *code* — it is a live webhook payload field and the first term of `_ref_price` in `webhook_handler.py`, which drives the margin size clamp, guaranteed-SL injection and the min-order-size estimate, and it is read by two dashboard-ui pages. Reusing it would have changed position sizing. A separate column, `orders.mark_price_at_decision`, was added instead (migration 070). |
