...
- **Flip live-path validation**: the one-way exchange flip path (an `open_long` that nets/closes an
  ...
  open legs.
- **social-listener stop management ("risk off the trade")**: partial profit-taking landed 2026-07-27
  (TRIM → `partial_close`, `social_pending_trims`), but the *other* half of a management card is still
  dropped. Post 9787-9790 said "Risk off the trade" alongside "Lock in W 64.4k" — move the stop to break
  even — and only the trim is acted on. The plumbing exists and needs no exchange work:
  order-listener already exposes `POST /strategies/{id}/adjust-stops` (same webhook token, body
  `{tp_price?, sl_price?}`) which routes to `call_executor_modify_stops`. Design: extract a
  `stop_price` / `stop_to_breakeven` field on TRIM and CLOSE-adjacent posts, and a
  non-state-changing `adjust_stops` decision in `evaluate()` alongside the trim branch. Open question
  worth settling first: whether a social post should ever be allowed to *widen* a stop, or only tighten
  one — widening is the only direction that can increase loss, so the safe default is tighten-only.
  Related and deliberately unbuilt: **ADD / scale-in** stays `is_actionable=false`, because sizing a
  scale-in needs its own margin decision that nothing downstream expresses.
