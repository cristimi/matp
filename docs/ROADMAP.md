...
- **Flip live-path validation**: the one-way exchange flip path (an `open_long` that nets/closes an
  ...
  open legs.
- **social-listener ADD / scale-in**: deliberately unbuilt. `extractor.py` forces `is_actionable=false`
  for ADD, because sizing a scale-in needs its own margin decision that nothing downstream expresses —
  unlike TRIM and stop moves (both shipped 2026-07-27), which only ever reduce exposure and so need no
  new capital rule. Building it means deciding what a social "adding here" is worth in margin terms:
  another full `margin_per_trade` unit, a fraction of the original, or a cap on total exposure per
  stance. Settle that before touching the extractor.
